...
class QuestionIndexViewTests(TestCase):
  def test_no_questions(self):
    response = self.client.get(reverse('gamification:index'))
  ...
  def test_future_question(self):
    ...
    response = self.client.get(reverse('gamification:index'))
    self.assertContains(response, "質問が利用可能ではない")
...
class QuestionDetailViewTests(TestCase):
  def test_future_question(self):
    ...
    url = reverse('gamification:detail', args=(future_question.id))
    response = self.client.get(url)
    self.assertEqual(response.status_code, 404)
  
  def test_past_question(self):
    past_question = create_question(question_text='過去の質問', days=-5)
    url = reverse('gamification:detail', args=(past_question.id,))
    response = self.client.get(url)
